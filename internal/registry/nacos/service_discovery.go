...
func (n *nacosServiceDiscovery) GetServices() *gxset.HashSet {
	...
	var pageNo uint32 = 0
	var pageSize uint32 = 2
	res := gxset.NewSet()
	for {
		tmp, _ := n.getServices(pageNo, pageSize)
		if tmp.Empty() {
			break
		}
		pageNo++
		for _, e := range tmp.Values() {
			res.Add(e)
		}
	}
	return res
...
func (n *nacosServiceDiscovery) getServices(pageNo, pageSize uint32) (*gxset.HashSet, int64) {
		...
		err := n.namingClient.Client().Subscribe(&vo.SubscribeParam{
			...
			SubscribeCallback: func(services []model.Instance, err error) {
				...
				for _, lis := range n.instanceListenerMap[serviceName].Values() {
					var instanceListener = lis.(registry.ServiceInstancesChangedListener)
