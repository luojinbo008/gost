...
func DefaultProxyImplementFunc(p *Proxy, v common.RPCService) {
	...
	makeCallProxy := func(methodName string, outs []reflect.Type) func(in []reflect.Value) []reflect.Value {
	...
	if err := refectAndMakeObjectFunc(valueOfElem, makeCallProxy); err != nil {
		logger.Errorf("The type or combination type of RPCService %T must be a pointer of a struct. error is %s", v, err)
		return
	}
}

func refectAndMakeObjectFunc(valueOfElem reflect.Value, makeCallProxy func(methodName string, outs []reflect.Type) func(in []reflect.Value) []reflect.Value) error {
	...
	for i := 0; i < numField; i++ {
		...
		if f.Kind() == reflect.Func && f.IsValid() && f.CanSet() {
			...
			f.Set(reflect.MakeFunc(f.Type(), makeCallProxy(methodName, funcOuts)))
			...
			if valueOfElemInterface.Type().Kind() == reflect.Struct {
				if err := refectAndMakeObjectFunc(valueOfElemInterface, makeCallProxy); err != nil {
					return err
				}
